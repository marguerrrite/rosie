...
Over the course of four days, we conducted initial customer interviews, collaborated via Zoom, Miro (RealtimeBoard), Slack, and Keynote, pen and paper'd, built a working prototype in Keynote, presented the dashboard to the original interviewees, and conducted user interviews with them.
...
The interviews would quickly let us know whether we were on the right track with a component or way off the mark. For example, initial iterations of the Stats sections listed each query out horizontally in a card. While this was a great way to view the information for individual queries, we saw that users had trouble comparing the information to each other, thus defeating the point of having complex queries!

###### Early prototype of Info Stats
![Early mockups of Info Stats](./currents-stats-mockups.png)

We also couldn't reliably get that much historical data into any given page query, so we had to scrap the heatmap.
...
- Either mockup, prototype in the code (or both) what that a potential solucion could look like (prototyping an initial version in the codebase was essential since would be able to work with real data samples)
- Present to the team and gather feedback
- Either polish and push to our staging platform for other company members to use and try out or go back to the drawing board to rinse and repeat
...
This ongoing project has been incredibly rewarding to be a part of&mdash;from working with the new data, building a completely new dashboard from scratch, collaborating with my amazingly talented teammates, and getting to learn _a lot_ along the way.
